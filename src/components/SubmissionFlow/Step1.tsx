    ...
    <div className="flex flex-col gap-2 md:grid md:grid-cols-2 md:gap-3">
      {types.map((t) => (
        <button
          ...
          className={`px-4 py-3 md:px-5 md:py-4 text-left border rounded-sm transition-all duration-300 group flex flex-row items-center gap-4 md:flex-col md:items-start md:gap-0 ${
            selectedType === t.id
              ? 'bg-stone-900 border-stone-900 text-white shadow-xl'
              : 'bg-white border-stone-200 hover:border-stone-400 text-stone-900'
          }`}
        >
          <t.icon className={`shrink-0 md:mb-3 ${selectedType === t.id ? 'text-stone-300' : 'text-stone-400 group-hover:text-stone-600'}`} size={22} />
          <div>
            <h3 className="text-sm md:text-base font-serif font-semibold md:font-normal mb-0.5 md:mb-1">{t.id}</h3>
